(function(){
  ...
  class App extends React.Component{
    constructor(p){super(p);this.state={data:loadData(),quote:false,menu:false,admin:location.hash.indexOf('#admin')===0,version:0,boot:true,announcement:null};this.onHash=this.onHash.bind(this);this.onScroll=this.onScroll.bind(this);this.onPointerMove=this.onPointerMove.bind(this);this.onPointerOut=this.onPointerOut.bind(this);}
    componentDidMount(){if(this.state.admin&&!shouldUseITopAdmin()&&!isLocalPreview()){goRealAdmin();return;}setTheme(this.state.data);window.addEventListener('hashchange',this.onHash);window.addEventListener('scroll',this.onScroll,{passive:true});document.addEventListener('pointermove',this.onPointerMove,{passive:true});document.addEventListener('pointerout',this.onPointerOut,{passive:true});this.initReveal();this.onScroll();var self=this,api=itopApi();if(api&&api.syncPublicProducts&&!this.state.admin){api.syncPublicProducts(this.state.data.products||[]).then(function(products){if(!products||!products.length)return;var d=clone(self.state.data);d.products=products;self.setState({data:d,version:self.state.version+1});}).catch(function(){})}setTimeout(function(){self.setState({boot:false});self.scheduleAnnouncement();},900);}
    componentDidUpdate(){setTheme(this.state.data);this.initReveal();}
    componentWillUnmount(){window.removeEventListener('hashchange',this.onHash);window.removeEventListener('scroll',this.onScroll);document.removeEventListener('pointermove',this.onPointerMove);document.removeEventListener('pointerout',this.onPointerOut);}
    ...
    scheduleAnnouncement(){var a=activePopup(this.state.data);if(!a||a.enabled===false||this.state.admin)return;var freq=a.frequency||'session';var key=ANNOUNCE_KEY+'-'+(a.id||'default');if(freq!=='everyVisit'){var seen=freq==='once'?storageGet('local',key):storageGet('session',key);if(seen)return;}var self=this;setTimeout(function(){self.setState({announcement:a});},Math.max(200,Number(a.delay||900)));}
    ...
    render(){var page=this.state.admin?h(Admin,{data:this.state.data,onSave:this.updateData.bind(this)}):h(Storefront,{data:this.state.data,onQuote:()=>this.setState({quote:true}),quote:this.state.quote,onCloseQuote:()=>this.setState({quote:false}),version:this.state.version});return h('div',{className:'app-root'},page,this.state.boot&&!this.state.admin&&h(BootOverlay),this.state.announcement&&!this.state.admin&&h(AnnouncementModal,{data:this.state.announcement,onClose:this.closeAnnouncement.bind(this)}));}
  }
  ...
  function Storefront(props){var d=props.data, products=(d.products||[]).filter(function(x){return x.enabled!==false}), featured=products.filter(function(x){return x.featured}).slice(0,6), cats=(d.categories||[]).filter(function(x){return x.enabled!==false}).slice(0,6), brands=(d.brands||[]).filter(function(x){return x.enabled!==false});return h('div',{className:'store'},
    ...
    h(Header,{data:d,onQuote:props.onQuote}),
  ...
  function Header(p){var d=p.data;return h('header',{className:'site-header'},h('div',{className:'header-inner'},
    h('a',{className:'brand-lockup',href:'#top'},h('img',{src:asset(d.site.logo),alt:'Tiến Bảo'}),h('span',null,h('b',null,'SƠN TIẾN BẢO'),h('small',null,'Sơn chính hãng'))),
    h('nav',{className:'desktop-nav'},['Sản phẩm','Bảng màu','Bảng giá','Tính lượng sơn','FAQ'].map(function(label,i){var id=['products','colors',null,'calculator','faq'][i];if(label==='Bảng giá')return h('a',{key:label,href:d.site.priceUrl},label);return h('button',{key:label,onClick:function(){scrollToId(id)}},label)})),
    h('div',{className:'header-actions'},h('a',{className:'round-btn',href:d.site.catalogUrl,title:'Sản phẩm'},'⌕'),h('a',{className:'btn magnetic btn-red header-quote',href:d.site.quoteUrl||'https://sontienbao.com/lien-he.html','aria-label':'Đi tới trang nhận báo giá Sơn Tiến Bảo'},'Nhận báo giá'))
  ));}
  ...
  class Calculator extends React.Component{
    constructor(p){
      ...
      this.state={area:125,coats:Number(p.data.calculator.defaultCoats||2),waste:Number(p.data.calculator.defaultWaste||10),productId:products[0]&&products[0].id,color:SWATCHES[0][0]};
    }
    calc(){
    ...
    render(){
      ...
      var form=h('div',{className:'calc-form'},
        ...
        h('div',{className:'two'},
          h('label',null,'Số lớp',h('select',{value:this.state.coats,onChange:e=>this.setState({coats:e.target.value})},[1,2,3].map(x=>h('option',{value:x,key:x},x+' lớp')))),
          h('label',null,'Hao hụt',h('div',{className:'input-suffix'},h('input',{type:'number',value:this.state.waste,onChange:e=>this.setState({waste:e.target.value})}),h('span',null,'%')))
        ),
        h('label',null,'Sản phẩm',h('select',{value:this.state.productId,onChange:e=>this.setState({productId:e.target.value})},products.map(p=>h('option',{value:p.id,key:p.id},p.name)))),
        h('label',null,'Màu tham khảo',h('div',{className:'swatch-line'},SWATCHES.map(s=>h('button',{type:'button',title:s[0],key:s[0],className:this.state.color===s[0]?'chosen':'',style:{background:s[1]},onClick:()=>this.setState({color:s[0]})}))))
      );
      var result=h('aside',{className:'calc-result'},
        h('span',{className:'calc-kicker'},'KẾT QUẢ DỰ KIẾN'),
      ...
      return h('section',{className:'calculator-section',id:'calculator'},
        h('div',{className:'container'},
          h(SectionHead,{eyebrow:'PAINT CALCULATOR',title:this.props.data.calculator.heading,desc:this.props.data.calculator.subheading}),
          h('div',{className:'calculator-shell'},form,result)
        )
      );
    }
  }
  ...
  class Colors extends React.Component{
    ...
    copy(code){if(navigator.clipboard&&navigator.clipboard.writeText)navigator.clipboard.writeText(code);this.setState({copied:code});var self=this;setTimeout(function(){self.setState({copied:''});},1100);}
  ...
  function Footer(p){
    ...
    return h('footer',null,
      h('div',{className:'container footer-grid'},
        ...
        h('div',null,h('h4',null,'Liên hệ'),h('a',{href:'tel:'+d.hotline},d.hotlineDisplay),h('a',{href:'mailto:'+d.email},d.email),h('p',null,d.address))
      ),
      h('div',{className:'copyright'},'© 2026 Sơn Tiến Bảo • Local-first React preview')
    );
  }

  class QuoteModal extends React.Component{
    constructor(p){super(p);this.state={name:'',phone:'',location:'',note:'',done:false};}
    submit(e){e.preventDefault();if(!this.state.name.trim()||!this.state.phone.trim())return alert('Vui lòng nhập họ tên và số điện thoại.');var a=itopApi();if(a&&a.isITopOrigin&&a.isITopOrigin()){var target=this.props.data.site.quoteUrl||'/lien-he.html';try{var u=new URL(target,location.origin);u.searchParams.set('name',this.state.name);u.searchParams.set('phone',this.state.phone);if(this.state.location)u.searchParams.set('location',this.state.location);if(this.state.note)u.searchParams.set('note',this.state.note);u.searchParams.set('source','landing-v7');location.href=u.pathname+u.search;return;}catch(err){location.href=target;return;}}if(!isLocalPreview()){var target=this.props.data.site.quoteUrl||'https://sontienbao.com/lien-he.html';try{var u=new URL(target,'https://sontienbao.com/');u.searchParams.set('name',this.state.name);u.searchParams.set('phone',this.state.phone);if(this.state.location)u.searchParams.set('location',this.state.location);if(this.state.note)u.searchParams.set('note',this.state.note);u.searchParams.set('source','landing-v7-github');location.href=u.toString();return;}catch(err){location.href='https://sontienbao.com/lien-he.html';return;}}var leads=loadLeads();leads.unshift({id:uid('lead'),name:this.state.name,phone:this.state.phone,location:this.state.location,note:this.state.note,status:'Mới',createdAt:new Date().toISOString(),source:'local-preview'});saveLeads(leads);this.setState({done:true});}
    render(){
      var content;
      if(this.state.done){
        ...
        content=h('form',{onSubmit:this.submit.bind(this)},
          ...
          h('label',null,'Họ tên *',h('input',{value:this.state.name,onChange:e=>this.setState({name:e.target.value})})),
          h('label',null,'Số điện thoại *',h('input',{value:this.state.phone,onChange:e=>this.setState({phone:e.target.value})})),
          h('label',null,'Tỉnh / Thành phố',h('input',{value:this.state.location,onChange:e=>this.setState({location:e.target.value})})),
      ...
      return h('div',{className:'modal-backdrop',onMouseDown:e=>{if(e.target===e.currentTarget)this.props.onClose();}},h('div',{className:'quote-modal'},h('button',{className:'modal-x',onClick:this.props.onClose},'×'),content));
    }
  }
