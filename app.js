(function(){
  ...
  function stopFrame(id){if(!id)return;if(window.cancelAnimationFrame)window.cancelAnimationFrame(id);else clearTimeout(id);}
  ...
  function loadData(){
    try{
      ...
      var currentCatalog=DEFAULT&&DEFAULT.meta&&DEFAULT.meta.catalogGeneratedAt;
      var savedCatalog=parsed&&parsed.meta&&parsed.meta.catalogGeneratedAt;
      if(currentCatalog&&savedCatalog!==currentCatalog)parsed.products=clone(DEFAULT.products||[]);
      var normalized=normalizeData(parsed);
      if(currentCatalog){
        normalized.meta.catalogGeneratedAt=currentCatalog;
        normalized.meta.catalogFamilies=Number(DEFAULT.meta.catalogFamilies||normalized.products.length);
        normalized.meta.catalogPackages=Number(DEFAULT.meta.catalogPackages||0);
      }
  ...
  function searchText(value){var text=String(value||'').toLowerCase();if(text.normalize)text=text.normalize('NFD').replace(/[\u0300-\u036f]/g,'');return text.replace(/đ/g,'d').replace(/\s+/g,' ').trim();}
  ...
  class App extends React.Component{
    constructor(p){super(p);this.state={data:loadData(),quote:false,quoteContext:null,menu:false,admin:location.hash.indexOf('#admin')===0,version:0,boot:false,announcement:null};this.scrollFrame=0;this.pointerFrame=0;this.pointerState=null;this.onHash=this.onHash.bind(this);this.onScroll=this.onScroll.bind(this);this.onPointerMove=this.onPointerMove.bind(this);this.onPointerOut=this.onPointerOut.bind(this);this.onKeyDown=this.onKeyDown.bind(this);}
    ...
    componentWillUnmount(){window.removeEventListener('hashchange',this.onHash);window.removeEventListener('scroll',this.onScroll);document.removeEventListener('pointermove',this.onPointerMove);document.removeEventListener('pointerout',this.onPointerOut);document.removeEventListener('keydown',this.onKeyDown);stopFrame(this.scrollFrame);stopFrame(this.pointerFrame);}
    onScroll(){if(this.scrollFrame)return;var self=this;this.scrollFrame=nextFrame(function(){self.scrollFrame=0;var y=window.scrollY||0;var max=Math.max(1,document.documentElement.scrollHeight-window.innerHeight);var prog=Math.max(0,Math.min(1,y/max));var bar=document.querySelector('.scroll-progress i');if(bar)bar.style.transform='scaleX('+prog+')';var header=document.querySelector('.site-header');if(header)header.classList.toggle('is-compact',y>60);var visual=document.querySelector('.hero-visual');if(visual&&y<window.innerHeight*1.15)visual.style.transform='translate3d(0,'+(y*.025)+'px,0)';});}
    ...
    onKeyDown(e){if(e.key==='Escape'&&(this.state.quote||this.state.announcement))this.setState({quote:false,quoteContext:null,announcement:null});}
    onPointerMove(e){if(window.matchMedia&&(window.matchMedia('(prefers-reduced-motion: reduce)').matches||!window.matchMedia('(hover: hover) and (pointer: fine)').matches))return;this.pointerState={target:e.target,x:e.clientX,y:e.clientY};if(this.pointerFrame)return;var self=this;this.pointerFrame=nextFrame(function(){self.pointerFrame=0;var p=self.pointerState,t=p&&p.target;if(!t||!t.closest)return;var spot=t.closest('.mouse-spotlight');if(spot){var r=spot.getBoundingClientRect();spot.style.setProperty('--mx',(p.x-r.left)+'px');spot.style.setProperty('--my',(p.y-r.top)+'px');}var mag=t.closest('.magnetic');if(mag){var b=mag.getBoundingClientRect(),dx=p.x-(b.left+b.width/2),dy=p.y-(b.top+b.height/2);mag.style.transform='translate3d('+(dx*.08)+'px,'+(dy*.08)+'px,0)';}});}
    ...
    render(){var page=this.state.admin?h(Admin,{data:this.state.data,onSave:this.updateData.bind(this)}):h(Storefront,{data:this.state.data,onQuote:context=>this.setState({quote:true,quoteContext:context||null,announcement:null}),quote:this.state.quote,quoteContext:this.state.quoteContext,onCloseQuote:()=>this.setState({quote:false,quoteContext:null}),version:this.state.version});return h('div',{className:'app-root'},page,this.state.boot&&!this.state.admin&&h(BootOverlay),this.state.announcement&&!this.state.admin&&h(AnnouncementModal,{data:this.state.announcement,onClose:this.closeAnnouncement.bind(this)}));}
  }
  ...
  function Storefront(props){var d=props.data, products=(d.products||[]).filter(function(x){return x.enabled!==false}), featured=products.filter(function(x){return x.featured}).slice(0,6), cats=(d.categories||[]).filter(function(x){return x.enabled!==false}).slice(0,6), brands=(d.brands||[]).filter(function(x){return x.enabled!==false});return h('div',{className:'store'},
    ...
    props.quote&&h(QuoteModal,{data:d,context:props.quoteContext,onClose:props.onCloseQuote})
  );}
  ...
  function ContactDock(p){var d=p.data.site;return h('aside',{className:'contact-dock','aria-label':'Liên hệ nhanh'},h('a',{href:'tel:'+d.hotline,title:'Gọi '+d.hotlineDisplay,'aria-label':'Gọi '+d.hotlineDisplay},h('span',null,h(Icon,{name:'phone'}))),h('a',{href:'https://zalo.me/'+d.zalo,target:'_blank',rel:'noopener',title:'Chat Zalo','aria-label':'Chat Zalo'},h('span',null,h(Icon,{name:'message'}))),h('button',{onClick:p.onQuote,title:'Nhận báo giá','aria-label':'Nhận báo giá'},h('span',null,h(Icon,{name:'fileText'}))));}
  ...
  class Calculator extends React.Component{
    ...
    layer(p,coats){var cov=Number(p&&p.coverage||0),area=Math.max(0,Number(this.state.area)||0),n=Math.max(1,Number(coats)||1),waste=Math.min(50,Math.max(0,Number(this.state.waste)||0)),qty=cov?area*n/cov*(1+waste/100):0,packs=this.pack(qty,p),pricing=this.packCost(p,packs);return{p:p||{},cov:cov,qty:qty,packs:packs,pricing:pricing,coats:n,unit:this.unitOf(p)};}
    ...
    singleItems(data){
      var q=searchText(this.state.singleQuery),brand=this.state.singleBrand;
      return this.allItems(data).filter(function(p){if(brand!=='all'&&String(p.brand||'')!==brand)return false;if(!q)return true;var hay=searchText((p.brand||'')+' '+(p.name||'')+' '+(p.category||''));return hay.indexOf(q)>=0;});
    }
    quotePayload(mode,layers){
      var items=(layers||[]).filter(function(r){return r&&r.p&&r.p.id;}).map(function(r){return{id:r.p.id,name:r.p.name||'',brand:r.p.brand||'',quantity:Number((r.qty||0).toFixed(1)),unit:r.unit||'L',coats:r.coats,cost:r.pricing&&r.pricing.known?r.pricing.total:null};});
      var known=items.length>0&&items.every(function(x){return Number(x.cost)>0;});
      return{source:'calculator',mode:mode,surface:this.surfaceLabel(),area:Math.max(0,Number(this.state.area)||0),waste:Math.min(50,Math.max(0,Number(this.state.waste)||0)),items:items,total:known?items.reduce(function(sum,x){return sum+Number(x.cost||0);},0):null};
    }
    render(){
      ...
      if(mode==='single'){
        ...
        var quoteLayers=[];if(showPrimer&&groups.primers.length)quoteLayers.push(primerR);if(showFinish&&groups.finishes.length)quoteLayers.push(finishR);var calculatorQuote=this.quotePayload(mode,quoteLayers);
        result=h('aside',{className:'calc-result calc-result-v3'},h('div',{className:'calc-result-topline'},h('span',{className:'calc-kicker'},mode==='system'?'KẾT QUẢ TOÀN BỘ HỆ SƠN':'KẾT QUẢ DỰ KIẾN'),h('span',{className:'surface-result-chip'},this.surfaceLabel())),h('div',{className:'calc-result-stack'},results),mode==='system'&&h('div',{className:'system-total'},h('div',null,h('span',null,'Tổng chi phí vật tư dự kiến'),h('small',null,'Tính theo giá đúng quy cách sản phẩm hiện có.')),h('strong',{className:totalKnown?'cost-known':'cost-pending'},totalKnown?money(totalCost):'Chưa đủ giá theo quy cách')),h(Btn,{kind:'red',className:'full',onClick:()=>this.props.onQuote(calculatorQuote)},'Nhận báo giá chính xác →'),h('small',{className:'estimate-note'},'Kết quả là ước tính hỗ trợ chọn mua dựa trên diện tích, khu vực sử dụng, quy cách và thông tin sản phẩm hiện có.'));
      }
  ...
  class Colors extends React.Component{
    ...
    render(){var all=(this.props.data.colors||[]).filter(function(x){return x.enabled!==false});var groups=['Tất cả'];all.forEach(function(x){if(x.group&&groups.indexOf(x.group)<0)groups.push(x.group);});var q=searchText(this.state.search),group=this.state.group;var items=all.filter(function(x){var ok=group==='Tất cả'||x.group===group;var text=searchText((x.code||'')+' '+(x.name||''));return ok&&(!q||text.indexOf(q)>=0);}).slice(0,12);return h('section',{className:'colors-section',id:'colors'},h('div',{className:'container color-grid'},h(Reveal,{className:'color-copy'},h('span',{className:'section-kicker'},'COLOR DISCOVERY'),h('h2',null,'Màu sắc không chỉ để chọn. Màu sắc là cách hình dung không gian.'),h('p',null,'Tìm theo mã hoặc tên màu, lọc theo nhóm và copy mã trước khi mở bảng màu chính thức.'),h('div',{className:'color-tools'},h('input',{className:'color-search',placeholder:'Tìm mã màu, ví dụ S 1515-R40B','aria-label':'Tìm mã hoặc tên màu',value:this.state.search,onChange:e=>this.setState({search:e.target.value})}),h('div',{className:'color-tabs'},groups.slice(0,7).map(g=>h('button',{type:'button',key:g,className:this.state.group===g?'active':'',onClick:()=>this.setState({group:g})},g)))),items.length?h('div',{className:'big-swatches color-explorer-grid'},items.map(c=>h('div',{className:'color-card mouse-spotlight',key:c.id||c.code,style:{background:c.hex||'#ccc'}},h('div',{className:'color-card-meta'},h('span',{className:'color-name'},c.name||c.group||'Màu sơn'),h('b',{className:'color-code'},c.code||'—')),h('div',{className:'color-card-actions'},h('button',{type:'button',onClick:()=>this.copy(c.code)},this.state.copied===c.code?'Đã copy':'Copy'),h('a',{href:this.props.data.site.colorUrl},'Xem →'))))):h('div',{className:'color-empty',role:'status'},h('b',null,'Không tìm thấy màu phù hợp'),h('span',null,'Thử mã màu, tên màu hoặc chọn nhóm khác.')),h('a',{className:'btn btn-dark magnetic',href:this.props.data.site.colorUrl},'Khám phá toàn bộ bảng màu →')),h(Reveal,{className:'color-room mouse-spotlight beam-card'},h('picture',null,h('source',{srcSet:'assets/color-room-v681-clean-768.webp 768w, assets/color-room-v681-clean.webp 1448w',sizes:'(max-width: 900px) 100vw, 52vw',type:'image/webp'}),h(SmartImage,{src:'assets/color-room-v681-clean.jpg',alt:'Không gian nội thất tham khảo với sơn Jotun',width:1448,height:1086,loading:'lazy'})),h('div',{className:'room-label'},h('b',null,'Không gian tham khảo'),h('span',null,'Màu hiển thị trên màn hình có thể khác màu thực tế.')))));}
  }
  ...
  class QuoteModal extends React.Component{
    ...
    render(){
      ...
      if(this.state.done){
        ...
        var estimate=this.props.context;
        content=h('form',{onSubmit:this.submit.bind(this)},
          ...
          estimate&&h('aside',{className:'quote-estimate','aria-label':'Tóm tắt kết quả tính sơn'},h('div',null,h('b',null,estimate.surface+' · '+estimate.area+' m²'),h('span',null,'Hao hụt '+estimate.waste+'%')),h('ul',null,(estimate.items||[]).map(function(x){return h('li',{key:x.id},h('span',null,x.name),h('strong',null,x.quantity>0?x.quantity+' '+x.unit:'Cần xác nhận'));})),h('div',{className:'quote-estimate-total'},h('span',null,'Chi phí dự kiến'),h('strong',null,Number(estimate.total)>0?money(estimate.total):'Cần xác nhận'))),
